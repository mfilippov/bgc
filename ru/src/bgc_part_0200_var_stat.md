...
Более точно, он показывает размер (в байтах), который занимает _type of a
particular expression_ (которое может быть просто одной переменной).
...
оба имеют тип `int`. К числу `4` мы вернёмся в следующем примере...
